...
function buildLessonTile(lesson, dateStr, startHour, pos, dayScale) {
  ...
  const tile = el('div', {
    className: `lesson-tile ${statusClass} ${isGroupLesson ? 'group-session' : ''} ${extraClasses.join(' ')}`.trim(),
    style: {
      top: `${top}px`,
      height: `${height}px`,
      left: leftStyle,
      width: widthStyle,
      right: 'auto',
      borderColor: tileGroupColor ? `${tileGroupColor}66` : undefined,
      background: tileGroupColor ? `linear-gradient(135deg, ${tileGroupColor}26, ${tileGroupColor}0c)` : undefined,
      overflow: 'hidden',
    },
    onClick: (e) => {
  ...
  if (tileGroupColor) {
    titleRow.appendChild(el('span', { className: 'tile-group-dot', style: { background: tileGroupColor } }));
  }
    ...
    for (const participant of participants) {
      ...
      if (showPackageCredits && participantPkg) {
        ...
        participantRow.appendChild(el('span', {
          ...
          style: { color }
        }, t('packageLabelShort', { count: participantPkg.credits })));
      }
